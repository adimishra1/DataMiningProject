...
for YEAR in range(2013,2017):
    ...
    for code in range(0,683):
        try:
            ...
            if pop > 0:
            	...
            	y.append(crime_total*100/pop)
            	# if crime_total>1.5:
            	# 	y.append(4)
            	# elif crime_total>0.6:
            	# 	y.append(3)
            	# elif crime_total>0.3:
            	# 	y.append(2)
            	# else:
            	# 	y.append(1)
    ...
    # fig, ax = plt.subplots()
    # # title for the plots
    # title = ('Decision surface of linear regressor ')
    ...
    # plt.xticks(())
    # plt.yticks(())
    # plt.title("Predicting crime based on literacy and Enrolment")
    # plt.show()
    # plot_contours(ax, clf, xx, yy, cmap=plt.cm.coolwarm, alpha=0.8)
    # ax.scatter(X0, X1, c=y, cmap=plt.cm.coolwarm, s=20, edgecolors='k')
    # ax.set_xlabel('lit_rate')
    # ax.set_ylabel('incentive')
    # ax.set_xticks(())
    # ax.set_yticks(())
    # ax.set_title('Decison surface using the ')
    # ax.legend()
    # plt.savefig("svm_classifier_literacy&incentive_vs_crime"+str(YEAR)+".png")
    # plt.clf()
